...
| **Group Rework v2: One Group Per User** | — | ✅ Done | Hard DB constraint (unique index on `group_members.user_id`). `GroupViewModel` members and `selectedGroupId` derived from `currentGroup` instead of `SelectedGroupHolder`; `selectGroup()`/`clearSelectedGroup()` removed. `SelectedGroupHolder` sync kept in init block for `ExpenseViewModel`/`MainViewModel` compatibility. ADR-0008. |
...
### #49 Trip Planning Module — implementation detail

| Sub-issue | Status | Notes |
|---|---|---|
| [#50](https://github.com/hllous/Proyecto-Plan-Travel/issues/50) Trip Planning foundations | ✅ Done | 5 DB migrations: destination columns on `travel_groups`, `itinerary_events`, `group_polls` (unique partial index), `poll_candidates`, `poll_votes` — RLS, REPLICA IDENTITY FULL, Realtime publication on each. New domain models: `PlaceResult`, `RankedRecommendations`, `ItineraryEvent`, `Poll`/`PollType`/`PollState`, `PollCandidate`. `PlacesApiClient` interface + `GooglePlacesApiClient` (Places New API v1) + Hilt module. `PlaceRecommendationRanker` with 5 TDD tests. `TravelRepository` extended with 11 methods for destination, itinerary and polls. |
| [#51](https://github.com/hllous/Proyecto-Plan-Travel/issues/51) Level 1 browse + set trip destination | ✅ Done | `DestinationViewModel` exposes region results, search results, `tripDestination` (reactive via `observeGroups`), `activePoll`, `currentMember`. `DestinationScreen`: region chips, search bar, card grid with `AsyncImage`, bottom sheet with photo/rating/reviews, "Establecer como destino" (ADMIN only), active-poll banner, auto-navigate to Level 2 when destination is set. `TravelGroupDto` updated with 4 destination columns. 5 TDD tests. |
| [#52](https://github.com/hllous/Proyecto-Plan-Travel/issues/52) Level 2 place recommendations by category | ✅ Done | `DestinationViewModel` adds `poisByCategory` and `selectPoiCategory()` — calls `PlacesApiClient.searchPois` + `PlaceRecommendationRanker`. `Level2Content`: category chips (Alojamiento, Gastronomía, Actividades, Naturaleza), `LazyColumn` with "Destacados"/"Otros" sections, `PoiCard` with `AsyncImage`, `PoiBottomSheet` with photo/rating/address/"Ver en Maps" deep link and "Añadir al itinerario"/"Añadir a encuesta" actions. 3 new TDD tests. |
| [#53](https://github.com/hllous/Proyecto-Plan-Travel/issues/53) Group Itinerary with day view | ✅ Done | `ItineraryViewModel`: events grouped by date, CRUD with `reloadTrigger`, `buildEventFromPoi`. `ItineraryScreen`: `LargeTopAppBar`, `LazyColumn` with sticky date headers, `EventCard` (edit/delete), FAB, `EventBottomSheet` with `DatePickerDialog`, empty state. `observeItineraryEvents` uses Postgres Changes + Broadcast fallback. Navigation accepts optional `draft` JSON arg for pre-fill from POI. 5 TDD tests. |
| [#54](https://github.com/hllous/Proyecto-Plan-Travel/issues/54) Group Poll | ✅ Done | `SupabaseTravelRepositoryImpl`: 7 poll methods with Postgres Changes + Broadcast fallback on `observePollCandidates`. `PollScreen`: empty state, open poll with real-time voting, closed poll with winner selection, creation bottom sheet. `DestinationScreen`: tappable `PollBanner`, "Añadir a encuesta" wired to `PollScreen`. `fetchActivePoll` fetches all states so CLOSED polls remain visible for winner selection. |
